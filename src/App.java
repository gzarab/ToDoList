...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        JTextArea entryArea = new JTextArea(); //single entry area
        entryArea.setText(myToDo.getEntry().toString());
        entries.add(entryArea);
        dayGrid.add(entryArea);
        for (int i = 0; i < 8; i++){
            JLabel textArea = new JLabel(); //8 display boxes
            textArea.setText(myToDo.get(i).getEntry().toString());
        ...
        saveButton.addActionListener(e ->  {
            try {
                    //String currentText = get(i).getText().trim();
